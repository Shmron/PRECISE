#!/usr/bin/env python3
"""
Generate Population-Weighted Road Network Density (PWRND) per admin unit.

  PWRND(unit) = sum(cell_RND * cell_pop) / sum(cell_pop)

Outputs (written to /var/www/precise/roadnet/data/pwrnd/):
  adm1.geojson  — provinces / states
  adm2.geojson  — districts
  adm3.geojson  — sub-districts

Usage:
  python3 generate_pwrnd.py                # process all 57 countries
  python3 generate_pwrnd.py ZWE KEN MOZ   # specific countries only
  python3 generate_pwrnd.py --no-download  # skip downloads (use cached rasters)
...
# ── Paths ──────────────────────────────────────────────────────────────────────
DATA_DIR    = '/var/www/precise/roadnet/data'
POP_DIR     = '/home/rutendo/PRECISE/roadnet/worldpop'
OUT_DIR     = os.path.join(DATA_DIR, 'pwrnd')
...
# ── Load RND data ──────────────────────────────────────────────────────────────
def load_country_rnd(iso3):
    path = os.path.join(COUNTRIES_DIR, f'{iso3}.json.gz')
...
def get_rnd(v):
    """Extract total RND from a cell value object."""
    return float(v.get('r', v.get('d', 0)) or 0)
...
def raster_to_h3_pop(tif_path, res=7):
    ...
    with rasterio.open(tif_path) as src:
        ...
        # Batch in chunks to show progress
        chunk = 200_000
...
def cells_to_geodataframe(rnd_data, cell_pop):
    """
    Build a GeoDataFrame of H3 cell centroids with RND and population columns.
    Only includes cells that have both RND > 0 and population > 0.
    """
    rows = []
    for cell_id, v in rnd_data.items():
        rnd = get_rnd(v)
        pop = cell_pop.get(cell_id, 0.0)
        if rnd <= 0 or pop <= 0:
            continue
        lat, lon = h3.cell_to_latlng(cell_id)
        rows.append({'cell_id': cell_id, 'rnd': rnd, 'pop': pop,
                     'geometry': Point(lon, lat)})
    if not rows:
        return gpd.GeoDataFrame(columns=['cell_id','rnd','pop','geometry'],
                                 crs='EPSG:4326')
    return gpd.GeoDataFrame(rows, crs='EPSG:4326')

def compute_pwrnd_for_level(cells_gdf, adm_gdf, name_col='name'):
    """
    Spatial join cell centroids to admin polygons, then compute PWRND per unit.
    Returns a copy of adm_gdf with 'pwrnd' and 'pop_sum' columns added.
    """
    if cells_gdf.empty:
        adm_out = adm_gdf.copy()
        adm_out['pwrnd']   = np.nan
        adm_out['pop_sum'] = 0.0
        return adm_out

    # sjoin: each cell → the admin polygon it falls within
    joined = gpd.sjoin(cells_gdf, adm_gdf[[name_col, 'geometry']],
                       how='inner', predicate='within')

    # Aggregate: PWRND = sum(rnd*pop) / sum(pop)
    agg = joined.groupby(name_col).apply(
        lambda g: {
            'pwrnd':   (g['rnd'] * g['pop']).sum() / g['pop'].sum() if g['pop'].sum() > 0 else np.nan,
            'pop_sum': g['pop'].sum(),
            'cell_n':  len(g),
        }
    ).apply(pd.Series) if False else None  # use manual approach below
    ...
    for name, group in joined.groupby(name_col):
        ...
        if total_pop > 0:
            grp[name] = {
                'pwrnd':   float((group['rnd'] * group['pop']).sum() / total_pop),
                'pop_sum': float(total_pop),
                'cell_n':  len(group),
            }

    adm_out = adm_gdf.copy()
    adm_out['pwrnd']   = adm_out[name_col].map(lambda n: grp.get(n, {}).get('pwrnd',   np.nan))
    adm_out['pop_sum'] = adm_out[name_col].map(lambda n: grp.get(n, {}).get('pop_sum', 0.0))
    adm_out['cell_n']  = adm_out[name_col].map(lambda n: grp.get(n, {}).get('cell_n',  0))
    return adm_out
...
def _write_outputs(adm_gdfs, adm_accum, adm_levels, out_dir):
    for key, _, _ in adm_levels:
        ...
        def lookup(idx, field):
            entry = acc.get(idx)
            if not entry or entry['den'] <= 0:
                return None
            if field == 'pwrnd':  return round(entry['num'] / entry['den'], 6)
            if field == 'pop':    return round(entry['den'])
            if field == 'cells':  return entry['cells']

        adm_gdf['pwrnd']  = adm_gdf.index.map(lambda i: lookup(i, 'pwrnd'))
        adm_gdf['pop']    = adm_gdf.index.map(lambda i: lookup(i, 'pop'))
        adm_gdf['cells']  = adm_gdf.index.map(lambda i: lookup(i, 'cells'))

        out_gdf = adm_gdf[adm_gdf['pwrnd'].notna() & (adm_gdf['cells'] >= 3)].copy()
...
def main():
    ...
    all_isos = sorted([f.split('.')[0] for f in os.listdir(COUNTRIES_DIR) if f.endswith('.json.gz')])
    ...
    adm_levels = [
        ('adm1', '/var/www/precise/roadnet/data/provinces.geojson',     'name'),
        ('adm2', '/var/www/precise/roadnet/data/districts.geojson',     'name'),
        ('adm3', '/var/www/precise/roadnet/data/sub_districts.geojson', 'name'),
    ]
    adm_gdfs = {}
    ...
    adm_accum = {key: {} for key in adm_gdfs}  # {key: {row_idx: {'num','den','cells'}}}

    for iso in isos:
        print(f'\n── {iso} ─────────────────────────────────────────')

        # Load RND data
        rnd_data = load_country_rnd(iso)
        if not rnd_data:
            print(f'  No RND data found, skipping')
            continue
        print(f'  {len(rnd_data):,} H3 cells')
        ...
        cells_gdf = cells_to_geodataframe(rnd_data, cell_pop)
        print(f'  {len(cells_gdf):,} cells with both RND>0 and pop>0')
        if cells_gdf.empty:
            continue
        ...
        for key, _, _ in adm_levels:
            ...
            for feat_idx, group in joined.groupby('index_right'):
                ...
                if total_pop <= 0:
                    continue
                weighted_rnd = float((group['rnd'] * group['pop']).sum())
                if feat_idx not in acc:
                    acc[feat_idx] = {'num': 0.0, 'den': 0.0, 'cells': 0}
                acc[feat_idx]['num']   += weighted_rnd
                acc[feat_idx]['den']   += total_pop
                acc[feat_idx]['cells'] += len(group)

        # Optionally free raster memory
    ...
    print('\nFinal write...')
    _write_outputs(adm_gdfs, adm_accum, adm_levels, OUT_DIR)
    print('\nAll done. PWRND files written to', OUT_DIR)
